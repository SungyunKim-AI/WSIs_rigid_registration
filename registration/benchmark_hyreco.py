...
from datasets import HyReCoDataset
...
class WSI_Registration:
    ...
    def run_registration(
        ...
        if self.save_result:
            landmarks_tgt = target['landmarks_ihc'] if self.reference == 'HE' else target['landmarks_he']
            landmarks_tgt_px = downscale_landmarks(landmarks_tgt, mpp_tgt)
            landmarks_tgt_px = landmarks_tgt_px.astype(int)
...
@click.command()
@click.option('--config', type=click.Path(exists=True), default="./configs/config_disk_hyreco.yaml")
def main(config: str) -> None:
    ...
    dataset = HyReCoDataset(cfg.data_dir, cfg.target_level, cfg.target_mpp)
    ...
    for idx in range(len(dataset)):
        ...
        tre_list = get_TRE_list(match_results['warped_landmarks'], target['landmarks_ihc'])   # unit: micrometer
        match_results['TRE_median'] = np.median(tre_list)
        match_results['TRE_mean'] = np.mean(tre_list)
        registrar.logger.info(f"\tTRE median: {match_results['TRE_median']:.4f}, TRE mean: {match_results['TRE_mean']:.4f}, Runtime: {match_results['runtime']:.4f} sec")
        
        for i, tre in enumerate(tre_list):
            tre_results.append({
                "slide_id": target['slide_id'], "reference":"HE", "target":target['stain'], 
                "landmark_index": i, 'target_landmark': target['landmarks_ihc'][i], 
                "warped_landmark": match_results['warped_landmarks'][i], "TRE": tre,
            })

        results.append({
            "slide_id": target['slide_id'], "reference":"HE", "target":target['stain'], 
            "TRE_median": match_results['TRE_median'], "TRE_mean":match_results['TRE_mean'], "num_matches": match_results['num_matches'], 
    ...
    df_tre = pd.DataFrame(tre_results)
    df_tre.to_csv(Path(cfg.output_dir)/"all_landmarks_results.csv", index=False)

    tre_median_list = df['TRE_median'].values
    tre_mean_list = df['TRE_mean'].values
    registrar.logger.info(f"Med TRE: {np.mean(tre_median_list):.4f} ({tre_median_list.std():.4f})")
    registrar.logger.info(f"Avg TRE: {np.mean(tre_mean_list):.4f} ({tre_mean_list.std()*2:.4f})")
