...
def warp_points(kpts, affine_matrix, inverse=False):
    """
    ...
    # 1. Convert to homogeneous coordinates (B, N, 2) -> (B, N, 3)
    ones = torch.ones((B, N, 1), device=device, dtype=dtype)
    points_h = torch.cat([kpts, ones], dim=-1)

    # 2. Extend affine matrix to 3x3 (B, 2, 3) -> (B, 3, 3)
    last_row = torch.tensor([[[0, 0, 1]]], device=device, dtype=dtype).expand(B, 1, 3)
    mat3x3 = torch.cat([affine_matrix, last_row], dim=1)

    if inverse:
        mat_to_use = torch.linalg.inv(mat3x3)
    else:
        mat_to_use = mat3x3

    # 3. Matrix multiplication
    # (B, N, 3) @ (B, 3, 3).T -> (B, N, 3)
    warped_points_h = torch.matmul(points_h, mat_to_use.mT)
    return warped_points_h[..., :2]


def warp_points_to_orig(kps, coord_map):
    """
    Convert image coordinates (kps) to original coordinates via coord_map (Identity Grid).
    Args:
        kps: (B, N, 2) or (N, 2) tensor [x, y]
        coord_map: (B, H, W, 2) or (H, W, 2) tensor [orig_x, orig_y]
    Returns:
        sampled_orig: (B, N, 2) or (N, 2) tensor
    """
    # Check if batch dimension is present
    kps_has_batch = len(kps.shape) == 3
    coord_has_batch = len(coord_map.shape) == 4
    
    if not kps_has_batch:
        kps = kps.unsqueeze(0)  # (N, 2) -> (1, N, 2)
    if not coord_has_batch:
        coord_map = coord_map.unsqueeze(0)  # (H, W, 2) -> (1, H, W, 2)
    
    B, N, _ = kps.shape
    _, H, W, _ = coord_map.shape
    
    # 1. Normalize kps to grid_sample format [-1, 1]
    grid_kps = kps.clone()
    grid_kps[:, :, 0] = (grid_kps[:, :, 0] / (W - 1)) * 2.0 - 1.0
    grid_kps[:, :, 1] = (grid_kps[:, :, 1] / (H - 1)) * 2.0 - 1.0
    grid_input = grid_kps.unsqueeze(1)  # (B, 1, N, 2)
    
    # 2. Reshape coord_map to (B, 2, H, W)
    map_input = coord_map.permute(0, 3, 1, 2)  # (B, H, W, 2) -> (B, 2, H, W)
    
    # 3. Sample original coordinates via bilinear interpolation
    sampled_orig = F.grid_sample(
        map_input, 
        grid_input, 
        mode='bilinear', 
        padding_mode='border', 
    ...
    # 4. (B, 2, 1, N) -> (B, N, 2)
    sampled_orig = sampled_orig.squeeze(2).permute(0, 2, 1)
    
    # Remove batch dimension if input had none
    if not kps_has_batch:
        sampled_orig = sampled_orig.squeeze(0)  # (1, N, 2) -> (N, 2)
    
    return sampled_orig


@torch.no_grad()
def gt_matches_from_affine(kp0, kp1, coord0, coord1, pos_th=3, neg_th=6, **kw):
    ...
    kp0_orig = warp_points_to_orig(kp0, coord0)
    kp1_orig = warp_points_to_orig(kp1, coord1)

    # build a distance matrix of size [... x M x N]
    dist = torch.sum((kp0_orig.unsqueeze(-2) - kp1_orig.unsqueeze(-3)) ** 2, -1)
    ...
    negative0 = dist.min(-1).values > neg_th**2
    negative1 = dist.min(-2).values > neg_th**2
