        ...
        for it, data in enumerate(train_loader):
            ...
            if (tot_it % conf.train.save_every_iter == 0 and tot_it > 0) and rank == 0:
                if results is None:
                    ...
                    best_eval = results[conf.train.best_key]
                save_matcher_pth(
                    model, output_dir, epoch, iter_i=tot_it, distributed=args.distributed
                )
            if stop:
                break

        if rank == 0:
            # Save LightGlue matcher as .pth each epoch
            save_matcher_pth(
                model, output_dir, epoch, iter_i=None, distributed=args.distributed
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--conf", type=str, default="02_LightGlue/configs/disk+lightglue_affine_patch.yaml")
    parser.add_argument(
        "--mixed_precision",
        "--mp",
        default=None,
    ...
    for module in conf.train.get("submodules", []) + [__module_name__]:
        mod_dir = Path(__import__(str(module)).__file__).parent
        dst_dir = output_dir / module
        
        # Exclude outputs directory
        def ignore_fn(dir, names):
            return ['outputs'] if 'outputs' in names else []
        
        if dst_dir.exists():
            shutil.rmtree(dst_dir)
        shutil.copytree(mod_dir, dst_dir, ignore=ignore_fn)
